...
@section('content')
    <div class="d-flex w-100 flex-column align-items-center justify-content-center mt-4">
        ...
        @endif
        <form method="POST" action="{{ route('admin.Page.Update') }}" class="w-75" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group mb-3">
                ...
                <textarea id="myTextarea" rows="10" class="form-control w-100 tinymce @error('content') is-invalid @enderror"
                    name="content" value="{{ old('content') }}" autocomplete="content">{{ $page->content }}</textarea>
                @error('content')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
