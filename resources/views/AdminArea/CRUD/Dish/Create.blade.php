...
@section('content')
    <div class="d-flex flex-column w-100 align-items-center justify-content-center mt-4">
        ...
        @endif
        <form method="POST" action="{{ route('admin.Dish.store') }}" class="w-75" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group mb-3">
                ...
                <select id="category" class="form-control headding @error('category_id') is-invalid @enderror"
                    name="category_id" value="{{ old('category_id') }}" required autocomplete="category_id">
                    <option value="-1" class="headding">اختر التصنيف</option>
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}" class="headding">{{ $category->name }}</option>
                    @endforeach
                </select>
                @error('category_id')
                    <span class="invalid-feedback" role="alert">
            ...
            <div class="form-group mb-3">
                <label for="" class="headding mb-2">التفاصيل</label>
